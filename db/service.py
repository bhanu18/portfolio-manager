...
async def update_asset(db: AsyncSession, db_asset: orm_models.Asset, asset_in: asset_schema.AssetUpdate):
    """
...
async def get_trade_by_id(db: AsyncSession, trade_id: int):
    ...
    query = select(orm_models.Trade).where(orm_models.Trade.id == trade_id).options(selectinload(orm_models.Trade.group).selectinload(orm_models.Group.members))
    result = await db.execute(query)
    return result.scalar_one_or_none()
...
async def update_trade(db: AsyncSession, db_trade: orm_models.Trade, trade_in: trade_schema.TradeUpdate):
...
async def get_all_users(db: AsyncSession, skip: int = 0, limit: int = 100):
    """Fetches all users."""
    query = select(orm_models.User).offset(skip).limit(limit).options(selectinload(orm_models.User.groups))
    result = await db.execute(query)
    return result.scalars().all()
...
async def add_or_update_user_in_group(
    ...
    association = next((assoc for assoc in user.group_associations if assoc.group_id == group.id), None)
